    ...
    def _register_mcp_servers(self, mcp_servers: dict) -> None:
        ...
        with _MCP_CONFIG_WRITE_LOCK:
            ...
            for server_name, server_config in mcp_servers.items():
                ...
                # WSL to Windows cross-environment bridge:
                # If mcp_config.json is stored on or shared with Windows (e.g. /mnt/c/),
                # Windows processes (Antigravity IDE, agy.exe) cannot execute Linux ELF binaries directly.
                # Wrap cao-mcp-server in a `wsl` invocation so Windows spawns it seamlessly via WSL stdio.
                resolved_str = str(path.resolve())
                if resolved_str.startswith(("/mnt/", "/c/")) or "\\wsl" in resolved_str:
                    if server_name == "cao-mcp-server" or "cao-mcp-server" in str(command):
                        distro = os.environ.get("WSL_DISTRO_NAME", "Ubuntu")
                        cao_home = os.environ.get("CAO_HOME_DIR", "$HOME/.cao")
                        entry["command"] = "wsl"
                        entry["args"] = [
                            "-d", distro,
                            "bash", "-c",
                            f'export PATH="$HOME/.local/bin:$PATH"; export CAO_HOME_DIR="{cao_home}"; export CAO_TERMINAL_ID="{self.terminal_id}"; cao-mcp-server'
                        ]
