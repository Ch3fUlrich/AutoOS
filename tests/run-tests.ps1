...
Test-Case 'autoos-agent plans tier runs without spawning or leaking a key' {
    ...
    Assert-True ($out -match '--standalone --agent tier2-worker --model omniroute/tier2 ') 'tier2 not paired with its model'
    $out = & $py.Source $tool run --tier 3 --clean --dry-run t 2>&1 | Out-String
    Assert-True ($out -match '--model omniroute/tier3-clean ') 'clean twin not chosen'
...
Test-Case 'tier depth is mandatory: only tier1 spawns, tier3 spawns nothing' {
    ...
    Assert-True ($t3.Count -gt 7) 'tier3 lost its fences'
    ...
    foreach ($rule in @($t3 | Where-Object { $_.action -like 'serena_*' -and $_.effect -eq 'allow' })) {
        Assert-True ($rule.action -notmatch 'create|replace|insert|rename|delete|edit|write|execute') "tier3 allows serena writer $($rule.action)"
    }
