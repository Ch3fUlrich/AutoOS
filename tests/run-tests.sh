...
problems = []
for tier, agent in ((1, "tier1-orchestrator"), (2, "tier2-worker"), (3, "tier3-reviewer")):
...
if it "autoos-agent: --clean picks the twin, undeclared models and --free --clean are refused"; then
    ...
    assert_contains "$out" "--model omniroute/tier3-clean "
...
# setting" and defaults to 1, so tier2 answered "Subagent depth limit reached
# (1)" when tier1 had launched it (live, 2026-09-24).
...
# never commits or pushes. Live 2026-09-24 the old stanza let tier3 commit
# through the shell and overwrite a file through serena's create_text_file.
if it "tier3-reviewer fences the shell, serena and omnigraph writes"; then
    report="$(python3 - <<'PY'
import json, re, io
oc = json.loads(re.sub(r"(?m)^\s*//.*$", "", io.open("opencode.jsonc", encoding="utf-8").read()))
fences = json.load(io.open("catalog/agent-harness.json", encoding="utf-8"))["fences"]
t3 = [(p["action"], p["resource"], p["effect"]) for p in oc["agents"]["tier3-reviewer"]["permissions"]]
problems = []
