...
| Qoder | NO CLI OAuth flow (absent from `oauth providers`) — dashboard Providers page only | Qwen3.8-Max-Preview (per dashboard ranking) | t2/t3 qwen overflow | connect via dashboard; then I list + probe |
...
| Devin CLI Agentic Bridge | `devin-cli-agentic` (noauth, alias `dva`) — needs the `devin` binary (`devin acp`); not on PATH here | `dva/*` already in the live catalog (opus-4-7-max, glm-5-2, kimi-k3, ... — no connection needed to list) | t1 candidate (opus-4-7-max) + GLM overflow | install binary (`https://cli.devin.ai/install.sh`) + `devin auth login`; then probe |
