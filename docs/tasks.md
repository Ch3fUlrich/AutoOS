...
| Routing v2 (`docs/plans/2026-09-25-routing-v2-plan.md`, phases 0, A–E) | autoos-L1-routing | 2026-09-25 | each phase merged to main with branch CI green | Running — phase 0, C1, client_key fallback (`a4eac1a`), B1–B3a + `--lane-mcp` (`7465fdd`), registry A1–A3 + skill v2 C2 (`36df121`), resolver B3b/B3c + `plan()` (`0ff2879`, `12c4624`), tool-calling probe + re-probe proposals (`d86711d`), probe 401/403 no verdict (`2bc52ee`), privacy close + RUNV2 + A4a/A4b renders (`c55f16c`), A4c + skill v3 (`8611bbd`), A4d/A4e renders + heartbeat rules->code (`a759555`), A5a/A5c registry readers (`3a3157f`), A5d installers + Q1 model policy as registry data (`95ba955`) merged; A8 done-pending-upstream (OmniRoute PRs #14900, #14901 from fork Ch3fUlrich/OmniRoute) |
| Backlog of run 1 (F5, F7, writer backups on Windows, OpenHands microagents, …) | autoos-L1-backlog | 2026-09-25 | list in `RUN/status/L1-backlog.md`, each item merged with CI green | Running — items 1, 2, 3 (F5), 5, 6 (`ai-stack.sh verify`) merged `d924b89`; Windows OpenCode V2, OpenHands skill mirror, Linux apt-key/backup hardening merged `5b3b14e`; Antigravity Hub tarball, OmniRoute qodercli image + public URL, Playwright idle-stop proxy, herdr-sessions component, same-second backup fixes merged `118fc7e`, fail-closed backups `aa53f31`, hostexec broker (nothing enabled) `dcb3ccc`, session-restore skip fix `82c1e0f`; run-tests.sh split next |
