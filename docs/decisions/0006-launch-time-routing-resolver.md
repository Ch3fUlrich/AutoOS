...
Status: accepted, trimmed · 2026-09-24 · evidence: facts re-verified below (tier1 research run +
...
| — | the renamed combo ids exist | **refuted**: no pushed repo carries them | `git ls-tree` of AutoOS and agent-skills `origin/main` |
...
1. **Build the resolver as one pure function** - `select_combo(card)` in one module that the
   ...
   the gateway does.
2. **Keep the tier ids.** `tier1/2/3`, `-clean`, `-credit` are a documented contract; the renames
   are rejected. A future rename needs one migration commit that updates `combos.json`, every
   client surface and the tests together - no alias table.
3. **Card v1** (unknown fields are an error):
   ...
   |---|---|---|---|---|
   | public | 1m | orchestrate, or hard | any | `tier1` |
   | public | 128k | implement / standard | free-ok | `tier2` (empty card) |
   | public | 128k | review, or trivial | free-ok | `tier3` |
   | public | 128k | implement or review | credit | `tier2-credit` / `tier3-credit` |
   | sensitive | 128k | implement / standard / hard | any | `tier2-clean` |
...
4. **Privacy is a hard filter, and there is no 1M private leg.** `tier1-clean` is paid-only but
   ...
   `tier2-clean` (128k), or pass an explicit `--allow-training` override, which is logged. It never
...
- **Q4 private implementer:** `tier2-clean` (DeepSeek paid first) by default; local Ollama only
  on explicit opt-in (it is not installed here, and this host has no RAM to spare).

## Review record

- Writer: Claude (L1). Reviewer: tier1 (`meta/muse-spark-1.3-contributor`, cross-family):
