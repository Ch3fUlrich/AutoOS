...
| `use-host-alias` | on a `local` host, `ssh`/`scp`/`sftp` always, `rsync` with a remote spec (`host:path`), and GNU `parallel` with `--sshlogin`/`-S`/`--sshloginfile`/`--transfer`/`--return` -- these skip the policy's host table (`forbid-host`, audit `host`); call `host_run` with `host=<alias>` instead -- checked on every command head |
